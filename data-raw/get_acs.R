...
acs_vars_ls$vehicles <-
  ...
  mutate(num_vehicles = case_when(
    num_vehicles == "No vehicle available" ~ "0",
  ...
  mutate(vehicle_available = case_when(
    num_vehicles == "0" ~ "0",
    num_vehicles == "1" ~ "1",
    TRUE ~ "2 or more"
  )) %>%
...
acs_summary_ls[["age"]] <-
  ...
  mutate(
    var_short = age,
    var_long = case_when(
      age == "Under 5" ~ "Children younger than 5 years old",
...
acs_summary_ls[["income"]] <-
  ...
  select(-income) %>%
  mutate(
    var_short = "Low-income households",
...
acs_summary_ls[["disability"]] <-
  ...
  mutate(
    var_short = case_when(
      disability_type == "ambulatory" ~ "Ambulatory difficulty",
      disability_type == "vision" ~ "Vision difficulty"
    ),
    var_long = case_when(
      disability_type == "ambulatory" ~ "People who have serious difficulty walking or climbing stairs",
...
acs_summary_ls[["vehicles"]] <-
  ...
  mutate(
    var_short = case_when(
      vehicle_available == "0" ~ "Zero-car households",
      vehicle_available == "1" ~ "One-car households"
    ),
    var_long = case_when(
      vehicle_available == "0" ~ "Households with no vehicle",
