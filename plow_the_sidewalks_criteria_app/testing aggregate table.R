...
summary_tab <-
  data.frame(
    ...
    num = c(
      ...
      prettyNum(round(as.numeric(summary$den), -2), big.mark = ","),
      prettyNum(round(as.numeric(summary$cta_permi2), -2), big.mark = ","),
...
summary_tab %>%
  ...
  gtExtras::gt_fa_column(icon,
                         height = "50px",
                         palette = rep(pal$access_purple, nrow(summary_tab))) %>%
